...
public class DuoTremDriver : MonoBehaviour
{
[Header("Primary tremolo Note")]
    ...
    void Awake()
    {
        if (!sender) sender = GetComponent<MidiSettingSender>();
        if (!sender) { Debug.LogError("DuoTremDriver: sender not found."); enabled = false; return; }

        _setting = sender.setting;
        if (!_setting) { Debug.LogError("DuoTremDriver: sender.setting missing."); enabled = false; return; }
